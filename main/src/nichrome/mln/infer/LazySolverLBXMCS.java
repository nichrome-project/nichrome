...
public class LazySolverLBXMCS extends LazySolver {
	...
	public LazySolverLBXMCS(MarkovLogicNetwork mln) {
		super(mln);
	}
	...
	private List<Set<GClause>> relaxConstraints(List<GClause> hardClauses,
		...
		try {
			...
			ProcessBuilder pb =
				new ProcessBuilder(Config.lbx_path, "-mxapp" ,"-T",
					Config.lbx_timeout + "", "-num", Config.lbx_numLimit
						+ "", consFile
						.getAbsolutePath());
	...
	class ResultInterpreter implements Callable<List<Set<GClause>>> {
		...
		public List<Set<GClause>> call() throws Exception {
			...
			while ((line = in.readLine()) != null) {
				System.out.println("LazySolverLBXMCS: "+line);
				this.log.append(line + "\n");
				timedout |= line.startsWith("c LBX timed out");
