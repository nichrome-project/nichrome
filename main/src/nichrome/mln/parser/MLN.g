...
@parser::header{
package edu.gatech.mln.parser;
import edu.gatech.mln.*;
import edu.gatech.mln.ra.*;
import edu.gatech.mln.util.*;
}
...
evidence : prior=NUMBER? NOT? pred=ID '(' terms+=(ID|NUMBER|STRING) (',' terms+=(ID|NUMBER|STRING))* ')'
    {
      ...
      if($prior != null){
	     ...
	     // TODO: currently we only support non-negtaive integer weight
		 // if(pr > 1 || pr < 0){
...
train : NOT? pred=ID '(' terms+=(ID|NUMBER|STRING) (',' terms+=(ID|NUMBER|STRING))* ')'
    {
      Boolean truth = null;
	  truth = ($NOT == null);
	    Predicate p = ml.getPredByName($pred.text);
	    ...
	    ArrayList<Token> ts = (ArrayList<Token>)($terms);
	    ArrayList<Integer> tuple = new ArrayList<Integer>();
      if(ts.size() != p.arity()) die("Line #" + (lineOffset+$pred.getLine()) + ": incorrect # args - " + $train.text);
      for(int i=0; i<p.arity(); i++){
          Token t = ts.get(i);
          Type type = p.getTypeAt(i);
          String s = t.getText();
