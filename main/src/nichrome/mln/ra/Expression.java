...
/**
 * An expression to a function is like a literal to a predicate. The interesting
 * part is that expressions can be nested. The value of an expression can be
...
final public class Expression implements Cloneable {
	...
	/**
	 * For aesthetics, check if we need a pair of parenthses for this
	 * experession.
	 *
	 */
	...
	public boolean isNumeric() {
		return this.func_.getRetType() == Type.Integer
			|| this.func_.getRetType() == Type.Float;
	}
	...
	public void addArgument(Expression expr) {
		if (this.args_.size() >= this.func_.arity()) {
			ExceptionMan.die("Function " + this.func_.getName() + " expected "
				+ this.func_.arity() + " arguments, but " + "received more");
		} else {
			this.args_.add(expr);
		}
	}
	...
	/**
	 * Bind variable references to their values in the symbol table. Variable
	 * name in the clause -> attribute name in SQL representing the value column
	 * of the symbol table.
	 *
	 * @param mapVarVar
	 */
	public String renameVariables(Map<String, String> mapVarVar) {
		String es = null;
		if (this.func_ == Function.VariableBinding) {
			String nvar = mapVarVar.get(this.val);
			if (nvar == null) {
				return this.val;
			} else {
				this.val = nvar;
	...
	public void bindVariables(Map<String, String> mapVarVal) {
		if (this.func_ == Function.VariableBinding) {
			String attr = mapVarVal.get(this.val);
			if (attr == null) {
				ExceptionMan
					.die("Encountered a dangling variable: " + this.val);
			} else {
				this.valBinding = attr;
			}
		} else {
			for (Expression e : this.args_) {
	 ...
	 */
	public String toSQL() {
		return this.getStringForm(true);
	}

	/**
	 * Compute the SQL representation of this expression. Call
	 * {@link #bindVariables(Map)} first.
	 *
	 */
	private String getStringForm(boolean inSQL) {
		...
		if (this.func_ == Function.VariableBinding) {
			return inSQL ? this.valBinding : this.val;
		}
		if (this.func_ == Function.ConstantString) {
			return inSQL ? SQLMan.escapeString(this.val) : "\""
				+ StringMan.escapeJavaString(this.val) + "\"";
		}
		...
		if (inSQL) {
			for (int i = 0; i < this.func_.arity(); i++) {
				Type t = this.func_.getArgTypes().get(i);
				String s = this.args_.get(i).toSQL();
				if (t == Type.Integer) {
					s =
				...
				if (this.func_ == Function.Eq || this.func_ == Function.Neq) {
					...
					// if(func_.getArgTypes().get(0) !=
					// func_.getArgTypes().get(1)){
			...
			for (int i = 0; i < this.func_.arity(); i++) {
				String s = this.args_.get(i).toString();
				if (this.args_.get(i).needEnclosure()) {
					s = "(" + s + ")";
				}
				sterms.add(s);
			}
		...
		sb.append((inSQL ? this.func_.getPgFunction() : this.func_.getName())
			+ "(");
		for (int i = 0; i < this.func_.arity(); i++) {
			sb.append(sterms.get(i));
			if (i != this.func_.arity() - 1) {
				sb.append(", ");
			}
		}
		sb.append(")");
	...
	public String toString() {
		return this.getStringForm(false);
	}

	/**
	 * Limited evaluation function, only support double type mapping and limited
	 * functions.
	 *
	 * @param varMap
	 * @return
	 */
	public boolean evaluate(Map<String, String> varMap) {
		if (this.func_ == Function.NOT) {
			return !this.args_.get(0).evaluate(varMap);
		}
		if (this.getArgs().size() != 2) {
			throw new RuntimeException(
				"The expression evaluation function currently only suports NOT and some binary functions.");
		}
		double val1 = this.lookUpOrCast(this.getArgs().get(0).val, varMap);
		double val2 = this.lookUpOrCast(this.getArgs().get(1).val, varMap);
		if (this.func_ == Function.LessThan) {
			return val1 < val2;
		}
		if (this.func_ == Function.LessThanEq) {
	...
	private double lookUpOrCast(String s, Map<String, String> varMap) {
		String looked = varMap.get(s);
		if (looked == null) {
			return Double.parseDouble(s);
		}
		return Double.parseDouble(looked);
	}
}
