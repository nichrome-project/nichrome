...
def to_k(y,position):
    ...
    if matplotlib.rcParams['text.usetex'] is True:
        return str(int(y/1000))+'\times 10^3'
    else:
...
def doPlot(client, configs, aspect_ratio=1):
    ...
    grid_size = (1,8)
    count = 0
    pPath = os.path.join(plotFolder,myUtils.getIterGridPlotFileName(client))
    gs = gridspec.GridSpec(1,8, wspace=0.26,hspace=0)

    fig.text(0.5, 0.04, '# questions', ha='center', fontsize=72)
    fig.text(0.095, 0.5, '# alarms', va='center', rotation='vertical',fontsize=72)
    for bench in benchmarks:
        ax = plt.subplot(gs[count/8,count%8])
        count+=1
        i = 0
        ...
        for classi in configs:
            ...
            if r is None:
                continue
            (x,y) = r
            ax.plot(x,y, color = colors[i], linestyle=linestyles[i], linewidth=6, alpha = 0.8)
            msize =800
            ax.scatter(x,y, s=msize, color = colors[i], marker=markers[i], label=configToLabel(classi),linewidth=6, alpha = 0.8, facecolors='none')
            i+=1
            maxX = max(maxX, max(x))
            maxY = max(maxY, max(y))
 ...
 #               ncol=2, mode="expand", borderaxespad=0.)
        if legend_bench == bench:
            lgd = plt.legend(loc=4, fontsize =48, borderpad=0.2)
       ...
       #         ha='left', va='bottom', fontsize=50, fontweight='bold')
        ax.text(0.5*x2, 1.01*y2, bench,ha='center', va='bottom', fontsize=60, fontweight='normal')

        if bench == 'sor' and client == 'pts':
            ax.text(0.49*x2, 0.5*y2, 'no false alarms',ha='center', va='bottom', fontsize=56)
