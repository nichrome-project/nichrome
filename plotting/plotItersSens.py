...
colors = ['fuchsia', 'brown', 'green', 'blue', 'red','black']
linestyles = ['-', '-', '-', '-', '-', '-','-']
markers = ['o', '|', '+', 'x', 's','^']
...
def doPlot(client, configs, aspect_ratio=1):
    ...
    for bench in benchmarks:
        f, ax = plt.subplots(1)
        i = 0
        pPath = os.path.join(plotFolder,myUtils.getSensivitiyIterPlotName(client,bench))
        ...
        for classi in configs:
            ...
            if r is None:
                continue
            (x,y) = r
            ax.plot(x,y, color = colors[i], linestyle=linestyles[i], linewidth=1, alpha = 0.8)
            msize = 20
            ax.scatter(x,y, s=msize, color = colors[i], marker=markers[i], label=configToLabel(classi),linewidth=1, alpha = 0.8, facecolors='none')
            i+=1
            maxX = max(maxX, max(x))
            maxY = max(maxY, max(y))
    ...
    #        lgd = plt.legend(loc=1, fontsize =16)
      ...
      #  y2 = normalize(maxY)
        ...
        if bench == legend_bench and x2 > ray_trace_x2:
            x2 = ray_trace_x2
        ...
        ax.set_ylabel('# alarms', fontweight='normal')
...
doPlot('datarace', raceConfigs, aspect_ratio=0.4)
