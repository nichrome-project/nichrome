...
def loadTime(client, bench, config):
    ifName = myUtils.getSolverTimeFileName(bench,config,client)
...
def doPlot(client, ymin =0, ymax=0, aspect_ratio=0.6, xlabel_rotate_angle=0, xmax = 0, useMin = False, useLog = False):
    ...
    if useMin:
        ...
        unit = 'seconds'
    ax.set_ylabel('runtime (in '+unit+')', fontweight='bold')

    plotName = myUtils.getSolverTimePlotFileName(client)
    plotPath = os.path.join(plotFolder, plotName)
    plt.savefig(plotPath, bbox_inches='tight')

doPlot('datarace', xlabel_rotate_angle=30, useMin=True, useLog=False)
doPlot('pts', xlabel_rotate_angle=30, useMin=True, useLog = False, ymax=100)
